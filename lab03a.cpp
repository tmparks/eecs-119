...
family of settlers moved to the island with their sheep, cows, goats and rabbits,
and the trouble started. Somehow, a few (ten) of the settlers' rabbits escaped
from their pen and formed a wild colony, which (lacking any natural enemies on
the island) grew very quickly.
...
of starvation. With these birth and death rates, the rabbit population would have
become stable at ten thousand, since:
...
Your assignment is to write a program that simulates the situation just described
and predicts the evolution of the two populations. The results generated could
then be used to predict whether the rabbit population would be reduced to
tolerable levels by the coyotes, and how long the settlers would have to wait
for those levels to be reached.
...
until the populations change less than 1% in a month, whichever comes first. (To
test this last condition for rabbits, say, for a month in which `r` rabbits are
alive and the change in rabbits is `rchange`, simply test whether `abs(rchange)/r`
is less than 0.01). The number of coyotes is zero until the beginning of the
fiftieth month when ten coyotes are introduced.
