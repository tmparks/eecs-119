...
where x is beiween 0 and 1. If you do not understand this expression, don't
...
thus a "sanill enough" error in the computation).

Your job is to write a
program that reads in x, computes I(x) using equation (2), and writes the
result. Your program should do the following:
...
or after 15 iterations, whichever comes first. When it stops, the programn
...
(3) For term n in the series, you wil need n! x^(2n+1). To compute these values,
